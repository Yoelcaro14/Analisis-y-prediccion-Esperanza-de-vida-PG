...
esperanza_vida= df[['ANIO',"PAIS","NOMBRE PAIS",'ESPERANZA']].drop_duplicates().reset_index(drop=True)
for pais in paises:
    esperanza_vida[esperanza_vida['PAIS']==pais] = esperanza_vida[esperanza_vida['PAIS']==pais][:-1]
esperanza_vida = esperanza_vida[:-1]
esperanza_vida=esperanza_vida.dropna()
esperanza_vida['ANIO']=esperanza_vida['ANIO'].astype(int)

df_final1 = pd.merge(df_kpi, esperanza_vida[['ANIO','PAIS','ESPERANZA']], how='left', left_on=['ds', 'id_pais'], right_on=['ANIO', 'PAIS']).drop(['ANIO','PAIS'],axis=1)
df_final1.drop_duplicates(inplace=True)
